...
class TermuxHelper {
    ...
    static getOptimizedConfig() {
        ...
        return {
            ...
            isProot,
            paths: {
                tempDir: isTermux ? `${process.env.PREFIX}/tmp` : require('os').tmpdir(),
                dataDir: isTermux ? `${process.env.PREFIX}/var/lib` : process.cwd()
            },
            memory: {
            ...
            network: {
                timeout: isTermux ? 60000 : 30000, // 增加 Termux 超时时间到 60 秒
                downloadTimeout: isTermux ? 120000 : 60000, // 下载专用超时时间
                retries: isTermux ? 3 : 3, // 增加 Termux 重试次数
                keepAlive: !isTermux, // Disable keep-alive in Termux
                pollingInterval: isTermux ? 5000 : 300,
                connectTimeout: isTermux ? 30000 : 15000 // 连接超时时间
            },
            features: {
